...
class StoreWalker
  ...
  def categories
    links_with(page, '#carroussel > div a').map { |link| CategoryWalker.new(link) }
  end
...
class CategoryWalker
  ...
  def categories
    links_with(page, '#blocCentral > div a').map { |link| SubCategoryWalker.new(link) }
  end
...
class SubCategoryWalker
  ...
  def items
    links_with(page, '#blocs_articles a.lienArticle').map { |link| ItemWalker.new(link) }
  end
