...
class DummyStoreItemsAPI
  ...
  def self.new_default_store(uri = Stores::Carts::DummyApi.url)
    new(full_config(uri))
  end

  def self.full_config(uri = Stores::Carts::DummyApi.url)
  ...
  def self.completed_config(config)
    config = { :uri => Stores::Carts::DummyApi.url }.merge(config)
    remote_id = 0
