* [DEVENV] migrate to ruby 2.0
** pass specs
** pass scenarios
** pass integration script
** deploy on heroku
** replace lazing by enumerable#lazy
* [DEVENV] speed up integration
** diminish the stores items api tests @range
** do less login-logout during cart testing
*** refactor the availble and 2 items prices tests (they are already tested in the before :all clause
** change my computer
** run things in parrallel
*** try running the (fast?) specs in // (try parallel_test gem)
*** try running cucumber scenarios in //
*** parrallel perl scripts replaces xargs and can dump outputs in block
*** use an integration server
*** migrate to github if it handles continuous integration
*** try the ruby "pty" library
*** try the parallel ruby gem
*** beware of specs and scenarios both using custom real_dummy_stores
...
* [HEROKU] create branches to track the versions deployed to heroku
