...
describe StoreItemsAPIBuilder do
  ...
  context "using define method" do
    it "creates new store api" do
      @builder = StoreItemsAPIBuilder.define(@api_class, StoreItemsDigger) { }

      @builder.new(@url).should == @api
    end

    it "initializes nested definition through its block" do
      StoreItemsAPIBuilder.stub(:new).and_return(builder = stub(StoreItemsAPIBuilder))

      builder.should_receive(:complex_builder_initialization)

      StoreItemsAPIBuilder.define(@api_class, StoreItemsDigger) do
  ...
  context "when nesting definitions" do
    ...
    before :each do
      @builder = StoreItemsAPIBuilder.new(@api_class, @digger_class)
    end
    ...
    [:categories, :items].each do |sub_definition|
      ...
      before :each do
        StoreItemsAPIBuilder.stub(:new).and_return(@sub_builder = stub(StoreItemsAPIBuilder))
