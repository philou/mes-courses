...
describe "layouts/application.html.erb" do
  ...
  it "should render a @path_bar element with no link" do
    text = "Recettes"
    assigns[:path_bar] = path_bar = [PathBar.element_with_no_link(text)]
    render
    response.should have_xpath("//div[@id='path-bar']/a[not(@href)][text()='#{text}']")
  end

  it "should display nicely a @path_bar with many elements" do
    assigns[:path_bar] = path_bar = [PathBar.element_with_no_link("Ingrédients"),
                                     PathBar.element_with_no_link("Marché"),
                                     PathBar.element_with_no_link("Poisson")]
    render
