...
describe "orders/show_success.html.erb" do
  ...
  it "renders a link to the online store" do
    response.should have_xpath("//a[@href='#{@order.store.sponsored_url}'][contains(.,'Aller vous connecter sur #{@order.store.name} pour payer')]")
  end
  ...
  it "renders an iframe to unlog the client from the remote store" do
    ...
    response.should have_xpath("//iframe[@id='remote-store-iframe'][@src='#{@order.store.logout_url}']")
