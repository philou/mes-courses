...
describe OrdersController do
  ...
  context "forwarding to a store" do
    ...
    it "should asynchronously pass the order" do
      ...
      delayed_job.should_receive(:pass).with(MesCourses::StoreCarts::Api.valid_login, MesCourses::StoreCarts::Api.valid_password)
    ...
    def forward_to_valid_store_account
      post 'create', :store_id => @store.id, :cart_id => @cart.id, :store => {:login => MesCourses::StoreCarts::Api.valid_login, :password => MesCourses::StoreCarts::Api.valid_password}
    end
